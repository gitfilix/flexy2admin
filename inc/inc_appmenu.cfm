...
<!--- alle Menues auslesen datasource application --->
...
<cfif isdefined("url.action") AND url.action EQ "submittedNewDish" AND isdefined("form.dish_name")>
		...
		INSERT	
		INTO	app_dish (rest_Name,isActive,dish_kind,dish_name,dish_image,dish_desc,mandant,placeid)
		VALUES(
			'#form.rest_name#',
			#form.active#,
			'#form.dish_art#',
...
<cfoutput query="editDish">
	...
	<tr>
		...
		<td>
			<select name="place">
				<cfloop query="get_allplaces">
					<option value="#id#" <cfif id EQ editdish.placeid >selected</cfif>> #place_name#</option>
				</cfloop>
			</select>
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<input type="hidden" name="app_dishid" value="#editdish.id#" />
