...
require.config({
	baseUrl: "/admin/inc/chat/js/",
	paths: {
		"domReady": "/admin/inc/chat/js/lib/require/domReady",
		"jquery": "/admin/inc/chat/js/lib/jquery/jquery-1.7.1",
		"order": "/admin/inc/chat/js/lib/require/order",
		"text": "/admin/inc/chat/js/lib/require/text",
	}
});
...
require(
	...
	function( $, ColdFusionWebSocket ){
		...
		logEvent( "Verbinden mit Webkanal." );
		...
		socket.on(
			...
			function( event, responseData ){
				...
				if (event.channel === "chat.userlist.subscribe"){
					...
					logEvent( 
						"Ein neuer Benutzer ist eingetreten [ " + 
						user.clientid + " ]." 
					);
					...
					logEvent( "Ein Benutzer hat den Chat verlassen" );
