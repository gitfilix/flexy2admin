...
<cfif isdefined("url.action") AND url.action EQ "submittedNewPlace" AND isdefined("form.place_name")>
		...
		INSERT	
		INTO	app_place (place_name,isActive,place_type,place_address,place_img,place_mandant)
		VALUES(
			'#form.place_name#',
			#form.active#,
			'#form.place_type#',
...
<!--- 	BEI JEDEM Binary fileupload muss im Formular multipart/form-data als enctype mitgegeben werden.  --->
	...
	<tr>
		...
		<td><input type="text" name="place_address"  id="place_address"  style="width:80%;"><cfif isdefined("form.place_address")>#place_address#</cfif></td>
	</tr>
	<tr>
	...
	<!---	wen die koordinate schon erfasst ist und form.vorname schon definiert ist -> prepopulate form.koordinaten --->
		<td><input type="text" name="coord_long"  style="width:80%;" <cfif isdefined("form.coord_long")>value="#form.coord_long#"</cfif>></td>
	</tr>
	<tr>
	...
	<!---	wen der user schon erfasst ist und form.vorname schon definiert ist -> prepopulate form.title --->
		<td><input type="text" name="coord_lat"  style="width:80%;" <cfif isdefined("form.coord_lat")>value="#form.coord_lat#"</cfif>></td>
	</tr>
	<tr>
		<td>Bild:</td>
		<td>
...
<cfoutput query="editPlace">
	...
	<tr>
		...
		<td>
			<cfif place_img NEQ "">
			Momentanes Bild auf dem Server: <a href="/#session.serverpath#/upload/img/app/#place_img#" target="_blank">#place_img#</a><br/>
			<input type="checkbox" name="delimage" value="#place_img#" /> Bild löschen<br/>
			</cfif>
			<input type="file" name="bild">
			<input type="hidden" name="origbild" value="#place_img#" />
		</td>
	</tr>
	<tr>
		<td></td>
